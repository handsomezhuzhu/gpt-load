...
func (s *KeyImportService) StartImportTask(group *models.Group, keysText string, concurrencyLimit int64) (*TaskStatus, error) {
	keys := s.KeyService.ParseKeysFromText(keysText)
	if len(keys) == 0 {
		return nil, fmt.Errorf("no valid keys found in the input text")
	}

	return s.StartImportTaskFromKeys(group, keys, uniformConcurrencyLimits(keys, concurrencyLimit))
}

// StartImportTaskFromKeys initiates a new asynchronous key import task with
// pre-parsed keys and per-key concurrency limits (e.g. group copy).
func (s *KeyImportService) StartImportTaskFromKeys(group *models.Group, keys []string, limitsByKey map[string]int64) (*TaskStatus, error) {
	if len(keys) == 0 {
		return nil, fmt.Errorf("no valid keys found in the input text")
	}
	...
	go s.runImport(group, keys, limitsByKey)

	return initialStatus, nil
}

func (s *KeyImportService) runImport(group *models.Group, keys []string, limitsByKey map[string]int64) {
	...
	addedCount, ignoredCount, err := s.KeyService.processAndCreateKeys(group.ID, keys, limitsByKey, progressCallback)
	if err != nil {
